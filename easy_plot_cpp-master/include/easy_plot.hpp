...
namespace easy_plot {
    ...
    public:
        ...
        // Строит тоже самое что и plot2, но только теперь он будет подписывать значения осей
        template <typename T1, typename T2>
        static int plot3(const std::string& name, const WindowSpec& wstyle, const std::vector<std::vector<T1>> x, const std::vector<std::vector<T2>> y, const std::vector<LineSpec>& style) {
            using namespace tools;
            {
                std::lock_guard<std::mutex> lock(drawings_mutex);
                int pos = get_pos_plot(name);
